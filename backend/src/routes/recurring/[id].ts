...
const recurringByIdRoute: FastifyPluginAsync = async (app) => {
  app.delete<{ Params: { id: string } }>('/:id', { preHandler: [app.verifyAccessToken] }, async (request, reply) => {
    ...
    try {
      await RecurringService.softDelete(dbUser.accountId, request.params.id)
...
export default recurringByIdRoute
